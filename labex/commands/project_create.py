...
class CreateProject:
    ...
    def create_project_md(self, path: str, gpt_model: str) -> None:
        ...
        if os.path.exists(json_path):
            ...
            exit(1)
        lab_content_prompt = f'Please revise the following content to a step-by-step tutorial as required. The tutorial only contains the Title, Introduction, Steps, and Summary. The title should begin with "# <Title>", and use Title Case. Introduction, Steps, Summary should begin with "## Introduction", "## Steps", and "## Summary". Each step should begin with "### Step X: <Step Title>", and the step title should be concise and clear. Each step should contain code blocks, and the code blocks must have code comments. Ensure the code is correct and can be executed. Each step should be explained in detail. The first step is to create the project files. The final step is how to run this project. Ensure the authenticity of the content and avoid fabrication. Be sure to include any necessary background information and avoid using technical jargon. Use markdown syntax to output the modified content. This tutorial needs to be completed so that students can complete it step by step.\n\n{lab_content}'
        print(f"[yellow]➜ PROMPTS:[/yellow] {lab_content_prompt}")
        step_raw_path = os.path.join(path, "step_raw.md")
        print(f"[yellow]➜ CREATE:[/yellow] {step_raw_path}")
